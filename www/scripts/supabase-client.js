...
/* ============================================================
   CACHE DE PERFIL PARA MOBILE/PWA
   ------------------------------------------------------------
   A autenticação oficial continua sendo a sessão do Supabase.
   Este cache existe apenas para repor, de forma síncrona, o
   `condominiumUser` quando navegadores mobile/PWA descartam o
   sessionStorage durante uma recarga/reabertura da aplicação.
============================================================ */
const CONDOMIT_PERSISTED_USER_V2_KEY = 'condomitPersistentUserV2';

function sanitizeCondomitUserForPersistence(user) {
  if (!user || typeof user !== 'object') return null;

  const safeUser = { ...user };

  // Nunca persistir credenciais ou tokens dentro do perfil da aplicação.
  ...
  safeUser.__persistVersion = 2;
  safeUser.__persistedAt = Date.now();
  return safeUser;
}

function persistCondomitUser(user) {
  const safeUser = sanitizeCondomitUserForPersistence(user);
  if (!safeUser) return false;

  try {
    localStorage.setItem(
      CONDOMIT_PERSISTED_USER_V2_KEY,
...
function clearPersistedCondomitUser() {
  try { localStorage.removeItem(CONDOMIT_PERSISTED_USER_V2_KEY); } catch (_) {}
}

function hydrateCondomitUserFromPersistence() {
  try {
    if (sessionStorage.getItem('condominiumUser')) return true;

    // Se o usuário clicou explicitamente em Sair, não restaurar a conta.
    if (localStorage.getItem('authExplicitLogoutAt')) return false;

    const raw = localStorage.getItem(CONDOMIT_PERSISTED_USER_V2_KEY);
    if (!raw) return false;

    const cached = JSON.parse(raw);
    if (!cached || cached.__persistVersion !== 2 || !cached.email) return false;

    const restored = { ...cached };
    delete restored.__persistVersion;
    delete restored.__persistedAt;
...
// Executa durante o carregamento do script, antes dos guards das páginas.
hydrateCondomitUserFromPersistence();

window.persistCondomitUser = persistCondomitUser;
window.clearPersistedCondomitUser = clearPersistedCondomitUser;
window.hydrateCondomitUserFromPersistence = hydrateCondomitUserFromPersistence;
...
async function refreshCurrentUserFromDb() {
  ...
  try {
    ...
    if (fresh) {
      ...
      try { persistCondomitUser(merged); } catch (_) {}
