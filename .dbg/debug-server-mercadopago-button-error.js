...
const session = 'mercadopago-button-error';
const outdir = path.join(process.cwd(), '.dbg');
const logFile = path.join(outdir, 'trae-debug-log-mercadopago-button-error.ndjson');
const envFile = path.join(outdir, 'mercadopago-button-error.env');

fs.mkdirSync(outdir, { recursive: true });
fs.writeFileSync(logFile, '');
fs.writeFileSync(envFile, 'DEBUG_SERVER_URL=http://127.0.0.1:7777/event\nDEBUG_SESSION_ID=mercadopago-button-error\n');
...
server.listen(7777, '127.0.0.1', () => {
  console.log(`debug-server http://127.0.0.1:7777 session ${session}`);
});
