...
function requireAssemblyCep() {
  ...
  if (!cep) {
    throw new Error(
      'Não foi possível identificar o CEP da assembleia.'
    );
  }
...
export function subscribeChat() {
  ...
  const channel =
    supabase
      .channel(
        `assembly-chat-${state.assemblyId}`
      )
      .on(
        'postgres_changes',
        {
          ...
          filter:
            `assembly_id=eq.${state.assemblyId}`
        },
        (payload) => {
          if (
            payload?.new
...
export async function sendChat(
  ...
  if (
    ...
    throw new Error(
      'Serviço de chat da assembleia indisponível.'
    );
  }
...
export function subscribeAgenda(
  ...
  state.agendaSubscription
    ?.unsubscribe?.();

  const channel =
    supabase
      .channel(
        `assembly-agenda-${state.assemblyId}`
      )
      .on(
        'postgres_changes',
        {
...
export function subscribeDocuments(
  ...
  state.docsSubscription
    ?.unsubscribe?.();

  const channel =
    supabase
      .channel(
        `assembly-docs-${state.assemblyId}`
      )
      .on(
        'postgres_changes',
        {
...
export function subscribePolls(
  ...
  state.pollsSubscription
    ?.unsubscribe?.();

  const channel =
    supabase
      .channel(
        `assembly-polls-${state.assemblyId}`
      )
      .on(
        'postgres_changes',
        {
...
export async function vote(
  ...
  if (
    ...
    throw new Error(
      'Esta votação já foi encerrada ou expirada.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Serviço de votação indisponível.'
    );
  }
...
export async function createPollWithDuration(
  payload,
  durationMinutes
) {
  if (
  ...
  if (
    ...
    throw new Error(
      'Título da votação deve ter ao menos 3 caracteres.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Informe ao menos 2 opções para a votação.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Informe uma duração válida para a votação.'
    );
  }
  ...
  if (!poll) {
    throw new Error(
      'Erro ao criar votação (sem retorno).'
    );
  }
  ...
  if (optsErr) {
    ...
    throw new Error(
      optsErr.message ||
      'Erro ao criar opções da votação'
    );
  }
...
export async function toggleHand(
  ...
  if (
    ...
    throw new Error(
      'Serviço de mão levantada indisponível.'
    );
  }
...
export async function loadHands() {
  ...
  if (error) {
    throw new Error(
      error.message ||
      'Erro ao carregar mãos levantadas'
    );
  }
...
export function subscribeHands(
  ...
  state.handSubscription
    ?.unsubscribe?.();

  const channel =
    supabase
      .channel(
        `assembly-hands-${state.assemblyId}`
      )
      .on(
        'postgres_changes',
        {
...
export async function refreshLists() {
  ...
  state._votesByPollAndUser =
    votesByPollAndUser;

  state._currentUserEmail =
    currentUserEmail;
